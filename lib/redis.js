       ...
       if (req.route.path == '/api/v1/contest-list/:match_id/:sport?/:series_id?') {
            ...
            try{
                ...
                if (req.userId) {
                    ...
                    promiseArray.push(redis.getPromiseForAnalysis(redisKeyForUserCategory, userCategory));
                }
                let redisData = await Promise.all(promiseArray).catch((err) => {
                ...
                if (redisData && redisData[0].length > 0) {
                    let matchContestData = JSON.parse(redisData[0]);
                    userCategory  = redisData && redisData.length >= 9 && redisData[8] ? JSON.parse(JSON.stringify(redisData[8])):userCategory;
...
const getBulkKeyRedisLeaderboard = (key, cb) => {
    ...
    const setRedisForUserAnaysis = (key, data, expTime = 432000, setExp = true) => {
        try {
            console.log('key** in set analysis redis', key)
     ...
     */
    const setRedisForUserCategory = (key, data, expTime = 432000000, setExp = fasle) => {
        try {
            redisForUserAnalysis.set(key, JSON.stringify(data));
            setExp && redisForUserAnalysis.expire(key, expTime);
        } catch (error) {
