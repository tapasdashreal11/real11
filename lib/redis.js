    ...
    try {
       ...
       if (req.route.path == '/api/v1/contest-list/:match_id/:sport?/:series_id?') {
            ...
            try{
                ...
                if(req && req.userId){
                 const resData = await redis.getPromiseForAnalysis(redisKeyForAddCashCoupon, "{}");
                 let couponData = resData ? JSON.parse(resData):{};
                 if(couponData && couponData.length>0){
                     console.log("couponData***",couponData);
                    return res.send(ApiUtility.success(couponData));
                 } else {
                    console.log("couponData empty***");
                    next();
                 }  
                } else {
                    return res.send(ApiUtility.failed("Wrong Parameters!!"));
