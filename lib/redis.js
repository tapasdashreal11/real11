...
var redisLiveFantasy = RedisClient.createClient(config.lf_redis.port, config.lf_redis.host);
 ...
 * @param expTime default 10h
 */
const setRedisForLf = (key, data, expTime = 432000, setExp = true) => {
    try {
        redisLiveFantasy.set(key, JSON.stringify(data));
        setExp && redisLiveFantasy.expire(key, expTime);
    } catch (error) {
        //console.error("setRedis error ==== ", error)
    }
}
...
const getRedisForLf = (key, cb) => {
    redisLiveFantasy.get(key, (err, data) => {
        if (err) {
            if (cb) {
                return cb(err)
            } else {
 ...
 * @param expTime default 10h
 */
const setRedisLogin = (key, data, expTime = 432000, setExp = true) => {
    try {
 ...
 * @param expTime default 10h
 */
const setRedisMyMatches = (key, data, expTime = 432000, setExp = true) => {
    try {
 ...
 * @param expTime default 10h
 */
const setRedisMyTeams = (key, data, expTime = 432000, setExp = true) => {
    try {
...
module.exports = {
    ...
    setRedisForLf,
    getRedisForLf, 
    redisLiveFantasyObj: redisLiveFantasy,
    setRedisLogin,
    getRedisLogin,
