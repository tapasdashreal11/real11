            ...
            try {
                ...
                if (redisData && redisData[0].length > 0) {
                    ...
                    if (req.userId) {
                        ...
                        console.log('*** countredis',redisData[3]);
                        finalRes['joined_predictions_count'] = jpc && jpc.length > 0 ? Helper.parseContestPredictionJoined(JSON.parse(jpc)) :[];
                        ...
                        finalRes['user_prediction_ids'] =userTeamIds && userTeamIds.length > 0 ? Helper.parseUserPrediction(JSON.parse(userTeamIds)):[];
                        finalRes['joined_contest_ids'] =redisData && redisData.length >= 6 && redisData[5] ? JSON.parse(redisData[5]):[];
