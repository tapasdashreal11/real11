...
const UserAnalysis = require("../server/models/user-analysis");
const FavouriteContest = require("../server/models/favourite-contest");
...
const cacheMiddle = async(req, res, next) => {
    try {
       ...
       if (req.route.path == '/api/v1/contest-list/:match_id/:sport?/:series_id?') {
            ...
            try{
                ...
                if (redisData && redisData[0].length > 0) {
                    ...
                    if (req.userId) {
                       ...
                       let retentionData = redisData && redisData.length >= 7 && redisData[6] ? JSON.parse(redisData[6]):{};
                       if(!retentionData && retentionData._id){
                        finalRes['user_rentation_bonous'] = retentionData;
                       } else {
                           console.log('data*****11');
                        ...
                        let userAnalysisData = await UserAnalysis.findOne({ user_id: req.userId, match_id: parseInt(req.params.match_id), sport: match_sport }, fileds);
                        if (userAnalysisData && userAnalysisData._id) {
                            ...
                            console.log('data*****12',userAnalysisData); 
                            setRedisForUserAnaysis(redisKeyForUserAnalysis, userAnalysisData);
                            finalRes['user_rentation_bonous'] = userAnalysisData;
                         }
                       }

                       let favouriteData = redisData && redisData.length >= 8 && redisData[7] ? JSON.parse(redisData[7]):{};
                       if(!favouriteData){
                        finalRes['user_favourite_contest'] = favouriteData;
                       } else {
                        console.log('data2222*****'); 
                        let userFavouriteConetsData = await FavouriteContest.findOne({ user_id: req.userId, status: 1 });
                        if (userFavouriteConetsData && userFavouriteConetsData._id) {
                            setRedisFavouriteContest(redisKeyForFavouriteContest, userFavouriteConetsData);
                            console.log('data2222*****',userFavouriteConetsData); 
                            finalRes['user_favourite_contest'] = userFavouriteConetsData;
                         }
                       }
            ...
            try{
                const {series_id, match_id, sport } = req.params;
                let cdataRsp = {};
