		...
		try {
			if (auth_user_id) {
				if (invite_code && !_.isEmpty(invite_code)) {
					...
					// 	if (ytuberCodeItem.applied) {
					// 		// use this when youtuber code is valid now
					// 		let ytuberUserData = await YoutuberUser.findOne({ user_id: ObjectId(auth_user_id), 'sport': parseInt(sport) });
					// 		if (ytuberUserData) {
					// 			response["message"] = "You have already applied the code.";
					// 			response["status"] = false;
					...
					// 		}
					// 	} else {
					// 		// use this when youtuber code has been expired
					// 		response["message"] = "This Code has been expired!!.";
					// 		return res.json(response);
					// 	}
