							...
							if (resToken.status == true && resToken.token) {
								bankVerification(params, user.phone, resToken.token, async function (veriyRes) {
									...
									if (veriyRes && veriyRes.status == true) {
										bankVerificationSync(params, veriyRes.token, resToken.token, async function (veriyResSync) {
											if (veriyResSync && veriyResSync.status == true) {
												...
												response["message"] = "Something went wrong at bank!!";
								                 return res.json({ response });
											}
										});
									} else {
...
async function bankVerification(bankData, phoneNo, token, cb) {
	...
	if (!_.isEmpty(bankData) && !_.isEmpty(token)) {
		...
		request(options, function (error, res, body) {
			...
			if (bodyRes && bodyRes.status == "SUCCESS" && bodyRes.data && bodyRes.data.bvRefId) {
