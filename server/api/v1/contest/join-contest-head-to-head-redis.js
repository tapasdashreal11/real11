                        ...
                        if (mtime < ctime) {
                            ...
                            if (teamId && teamId != null && teamId != '' && !_.isUndefined(teamId) && teamCount > 0) {
                                ...
                                if (matchContest && matchContest.category_slug && (_.isEqual(matchContest.category_slug, 'head-to-head') || _.isEqual(matchContest.category_slug, 'last-man-standing')) && match_sport == 1) {
                                    ...
                                    var PlayerTeamContestFilter = { 'contest_id': contest_id, 'user_id': user_id, 'match_id': decoded['match_id'], 'sport': match_sport, 'player_team_id': teamId }
                                    let playerTeamRes = await PlayerTeamContest.findOne(PlayerTeamContestFilter);
                                    let joinedContestWithTeamCounts = await PlayerTeamContest.find({ 'contest_id': contest_id, 'user_id': user_id, 'match_id': decoded['match_id'], 'sport': match_sport }).countDocuments()
                                    // let joinedContestWithTeamCounts = results[2] ? results[2] : 0;
                                    // Calculate the max team size of contest
                                    let maxTeamSize = contestData && contestData.maximum_team_size && !_.isNull(contestData.maximum_team_size) ? contestData.maximum_team_size : 1;
                                    /**
                                     * Check max team size. It should not more then expected size of contest
                                     * Otherwise we send msz to user for not join with more teams in this contest
                                     */
                                    if (joinedContestWithTeamCounts < maxTeamSize) {
                                        /**
                                         * We check if this contest user already joined or not
                                         */
                                        if (!playerTeamRes) {
                                            ...
                                            /**
                                             * This is the main section to join the contest. First we check joined count from redis and then 
                                            ...
                                            redis.redisObj.get(mcontestIncKey, async (erri, dataInc) => {
                                                ...
                                                if (mIcount < contestData.contest_size && infinteStatus) {
                                                    let totalTeamJoinedCount = 1;
                                                    let mcRedisIncData = totalBeforJoin
                                                    if (mcRedisIncData <= contestData.contest_size && infinteStatus) {
                                                        const doc = await MatchContest.findOne({ 'match_id': decoded['match_id'], 'sport': match_sport, 'contest_id': contest_id });
                                                        try {
                                                            if (doc) {
                                                                let joinedContestCount = doc.joined_users;
                                                                ...
                                                                let joinStatus = joinedContest && (joinedContest < contestData.contest_size || contestData.infinite_contest_size == 1) ? true : (joinedContest == 0 ? true : false);
                                                                ...
                                                                if (joinStatus == true) {
                                                                    ...
                                                                    try {
                                                                        ...
                                                                        if (contestType == "Free" || (contestType == "Paid" && totalEntryAmount > 0 && calEntryFees > 0 && totalEntryAmount == calEntryFees && userWalletStatus) || (calEntryFees == 0 && retention_bonus_amount > 0 && userWalletStatus)) {
                                                                                    ...
                                                                                    try {
                                                                                        if (_.has(contest, "player_team_id") && _.has(contest, "team_count") && _.has(contest, "team_name") && contest.team_name != '' && contest.player_team_id != null && contest.player_team_id != '' && contest.team_count != null && contest.team_count != '' && contest.team_count > 0) {
                                                                                            ...
                                                                                            if (mcRedisJoinedInc <= contestData.contest_size && infinteStatus) {
                                                                                                ...
                                                                                                joinedContestCount = docNew.joined_users;
                                                                                                if (contestData && contestData.contest_size < joinedContestCount && infinteStatus) {
                                                                                        ...
                                                                                        console.log("erorr single jc", ssError);
                                                                                        await session.abortTransaction();
                                                                                        session.endSession();
                                                                                        if (isIncrBy) {
                                                                                            await redis.redisObj.decrby(mcontestIncKey, totalTeamJoinedCount);
                                                                            ...
                                                                            // to call set data to user category
                                                                            ...
                                                                            if (playerContest.id) {
                                                                                if (contestData) {
                                                                                    /**
                                                                                     * This section is used to set team counts after team joined
                                                                                     */
                                                                                    ...
                                                                                    /**
                                                                                     * This is for user reset the data on the list.
                                                                                     */
                                                                                    ...
                                                                                    //Set to My Match data after contest join in redis 
                                                        ...
                                                        return res.send(ApiUtility.failed("Contest is full.You can't join this contest.!!"));
                                                    }
                                                } else {
                                                    return res.send(ApiUtility.failed("This contest is full.Please join another contest!!"));
                                                }
...
async function joinContestGlobal(res, refer_by_user, refer_code, joinedContestCount, indianDate, decoded, contestData, series_id, prms_contest_id, match_id, parentContestId, match_sport, liveMatch, teamId, user_id, teamCount, authUser, results, prms_matchContest, matchContestData) {
    ...
    try {
        ...
        if (mResultData && mResultData._id && parentContestId && mResultData.parent_contest_id) {
            ...
            let contest = {};
            let newContestId = new ObjectId();
            let mcontestIncKey = 'mci_' + match_id + '_' + match_sport + '_' + matchContest.contest_id;
            contest._id = newContestId;
            contest.player_team_id = teamId;
            contest.match_id = match_id;
            contest.series_id = series_id;
            ...
            try {
                ...
                if (contestType == "Free" || (contestType == "Paid" && totalEntryAmount > 0 && calEntryFees > 0 && totalEntryAmount == calEntryFees && userWalletStatus) || (calEntryFees == 0 && retention_bonus_amount > 0 && userWalletStatus)) {
                    try {
                        ...
                        /**
                         * This is used to set joined team counts in contest.
                         */
                        let joinedTeamsCountKey = `${RedisKeys.CONTEST_JOINED_TEAMS_COUNT}${match_id}`;
                        redis.getRedis(joinedTeamsCountKey, (err, data) => {
                            if (data) {
                        ...
                        /**
                         * This is used to reset data in redis
                         */
                        redis.redisObj.get('user-teams-count-' + match_id + '-' + match_sport + '-' + user_id, (err, data) => {
                            let count = (data) ? parseInt(data) : 1;
                            // mqtt.publishUserJoinedTeamCounts(match_id,user_id,JSON.stringify({team_count:count}))
                        ...
                        redis.setRedisMyMatches(matchContestUserKey, []);
                        /**
                         * This is used to set join contest data to appsflyer
                         */
                        setJoinContestDataToAppsflyer(authUser, match_id, sport, contest_id);

                        /**
                         * This is final response after contest join
                         */
                        return res.send(ApiUtility.success(data1, 'Contest Joined successfully.'));
            ...
            return res.send(ApiUtility.failed("Contest data not found.Please try again."));
        }
    } catch (elseError) {
...
async function contestAutoCreateAferJoin(isCommit, isPrivateCreate, contestData, series_id, contest_id, match_id, parentContestId, match_sport, liveMatch, session, matchContest) {
    try {
        ...
        if (cResult && cResult._id) {
            ...
            entityM.joined_users = !isCommit ? 1 : 0;
...
async function setJoinContestDataToAppsflyer(authUser, match_id, sport, contest_id) {
    try {
...
/**
 * This is used to set user category data in redis after joined contest
 * @param {*} contestData 
 * @param {*} authUser 
 * @param {*} user_id 
 */
async function setCategoryUser(contestData, authUser, user_id) {
    try {
        ...
        console.log("Jc User Category err", errrrrr);
    }
}