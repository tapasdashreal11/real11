...
module.exports = {
    joinedContestListUpcomingEnt: async (req, res) => {
        try {
            ...
            if (match_id && series_id && user_id) {
                ...
                let userTeamKey = `${redisKeys.USER_CREATED_TEAMS}${match_id}-${sport}-${user_id}`;
                let s3Key = match_id+"_"+sport+"/"+match_id + "_" + sport + "_" + user_id + "_";
                let getData = [];
                try {
                    getData = await redisEnt.getHashRedis(userTeamKey) //data from redis
                ...
                if (joindedContestlistdata && joindedContestlistdata.joined_contest) {
                    ...
                    let myTeams = 0;
                    if(getData) {
                        myTeams =   getData.length;
                    } else {
                        myTeams = await PlayerTeam.find({ 'user_id': decoded['user_id'], 'match_id': decoded['match_id'] }).countDocuments();
                    }
                    ...
                    if (ptcData && ptcData.length > 0) {
                        ...
                        let playerTeamData  =   "";
                        if(getData) {
                            playerTeamData  =   getData
                        } else {
                            playerTeamData  =   PlayerTeam.find({ _id: { $in: playerteamIds } }).exec()
                        }
                        ...
                        if (ptAndContestData && ptAndContestData.length > 0) {
                            ...
                            for (const ptcDataItem of ptcData) {
                                var joinObj = {};
                                const ptObj = _.find(playerTeamList, { '_id': ptcDataItem.player_team_id.toString() });
