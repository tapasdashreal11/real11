...
const { RedisKeys, MatchStatus } = require('../../../constants/app');
...
module.exports = {
    joinedContestMatchesNewRedisEnt: async (req, res) => {
        try {
            ...
            if (decoded) {
                if (decoded['user_id']) {
                    ...
                    if (decoded.is_complete == 'true') {
                        ...
                        redisEnt.getNormalFunRedis(matchContestKey, function (err, contestData) { // Get Redis 
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (type == 'upcoming') {
                ...
                queryObj = {
                    sport: sport, status: 1, time: { $gte: pastMonthDateCM }, $or: [
