...
async function switchTeamFn(id, team_id,count, sport, decoded, parentContestId) {
    try {
        if(!_.isNull(team_id) && !_.isNull(id)){
            ...
            var ptcDataS3 = process.env.S3_FOLDER_PLAYER_TEAM_CONTEST+"/"+decoded['match_id']+"_"+sport+"/"+parentContestId+"_"+decoded['contest_id']+"_"+decoded['user_id']+"_"+id+".json";
            var s3SingleTeamData = await s3Helper.multipleDataS3Bucket(ptcDataS3);
            s3SingleTeamData[0].player_team_id = team_id;
            s3SingleTeamData[0].team_count = count;
            await s3Helper.savePTCDataArrOnS3(s3SingleTeamData, "H2H");
        }
...
module.exports = {
    switchTeamRedisEntNew: async (req, res) => {
        try {
            ...
            if (decoded['user_id'] && decoded['match_id'] && decoded['series_id'] && decoded['contest_id'] && decoded['team_id']) {
                ...
                if (authUser) {
                    ...
                    if (liveMatch) {
                        ...
                        let contestData = await Contest.findOne({ _id: ObjectId(contest_id) });
                        ...
                        if (decoded['team_id'] && decoded['team_id'].length > 0) {
                            ...
                            var ptcDataMultiS3 = process.env.S3_FOLDER_PLAYER_TEAM_CONTEST+"/"+match_id+"_"+sport+"/"+parentContestId+"_"+contest_id+"_"+user_id;
                            var pleasrTeamData = await s3Helper.multipleDataS3Bucket(ptcDataMultiS3);
                            ...
                            _.forEach(pleasrTeamData, async function (i, k) {
                                ...
                                await switchTeamFn(i._id, decoded['team_id'][k],count, sport, decoded, parentContestId);
                                ...
                                if (k === (decoded['team_id'].length - 1)) {
                                    ...
                                    if (matchContest && matchContest.category_slug && (_.isEqual(matchContest.category_slug, 'head-to-head') || _.isEqual(matchContest.category_slug, 'last-man-standing'))) {
                                       if(matchContest.is_full){
                                            ...
                                            //     contest_id: decoded['contest_id'] ,
                                            // }).limit(10).sort({ _id: -1 });
                                            var ptcDataS31 = process.env.S3_FOLDER_PLAYER_TEAM_CONTEST+"/"+decoded['match_id']+"_"+sport+"/"+parentContestId+"_"+decoded['contest_id'];
                                            let allTeamsArr = await s3Helper.multipleDataS3Bucket(ptcDataS31);
                                            allTeams = _.sortBy(allTeamsArr, [function(o) { return o._id; }]);
                                            allTeams = allTeams.slice(0, 10);
