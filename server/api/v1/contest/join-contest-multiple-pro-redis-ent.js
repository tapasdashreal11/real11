    ...
    try {
        ...
        if (match_id && series_id && contest_id && user_id && team_data && _.isArray(team_data) && team_data.length > 0) {
            ...
            let apiList = [
                ...
                S3Helper.getPTCCountS3(ptcCountKey), //PlayerTeamContest.find({ 'contest_id': contest_id, 'user_id': user_id, 'match_id': decoded['match_id'], 'sport': match_sport, 'series_id': decoded['series_id'] }).countDocuments(),
                    ...
                    if (liveMatch) {
                        ...
                        if (mtime < ctime) {
                            ...
                            if (team_data && team_data != null) {
                                ...
                                var ptcCountKeyContestBased = process.env.S3_FOLDER_PLAYER_TEAM_CONTEST+"/"+match_id+"_"+match_sport+"/"+contest_id;
                                let joinedContest = await S3Helper.getPTCCountS3(ptcCountKeyContestBased)
                                ...
                                var ptcCountKeyUserBased = process.env.S3_FOLDER_PLAYER_TEAM_CONTEST+"/"+match_id+"_"+match_sport+"/"+contest_id+"_"+user_id;
                                var ptcDataUserBased = await S3Helper.multipleDataS3Bucket(ptcCountKeyUserBased)
                                let playerTeamRes = ptcDataUserBased.filter(a => tIds.includes(a.player_team_id));
                                    ...
                                    if (playerTeamRes && playerTeamRes.length == 0) {
                                        ...
                                        try {
                                            ...
                                            for (const team_data_item of teamArray) {
                                                ...
                                                contest.match_id = parseInt(match_id);
                                                contest.series_id = parseInt(series_id);
                                                    ...
                                                    if (contestData.contest_size > joinedContest) {
                                                        ...
                                                        let remainteamCount   = joinedContestCount - total_team_number;
                                                         if(remainteamCount != joinedContest){
                                                            ...
                                                            var ptcCountKeyNew = process.env.S3_FOLDER_PLAYER_TEAM_CONTEST+"/"+match_id+"_"+match_sport+"/"+contest_id;
                                                            joinedContest = await S3Helper.getPTCCountS3(ptcCountKeyNew);
            ...
            // await PlayerTeamContest.create(contest, { session: session }).then(async (newDataPTC) => {
                ...
                if (isAutoCreateStatus) {
                    if (joinedContestCount == contestData.contest_size) {
                        ...
                        contestAutoCreateAferJoin(contestData, series_id, contest_id, match_id, parentContestId, match_sport, liveMatch, session, matchContest, joinedContestDetailsArr);
                        ...
                        if(!_.isEmpty(joinedContestDetailsArr)) {
                            let s3keyArr = [];
                            let rediskeyArr = [];
                            for(const jcd of joinedContestDetailsArr) {
                                ...
                                if(s3Res) {
                                    let redisKeyJCD = `${RedisKeys.JOINED_CONTEST_DETAILS}${jcd.match_id}-${jcd.sport}-${jcd.user_id}-${jcd.contest_id}`;
                                    rediskeyArr.push(redisKeyJCD);
                                    redisEnt.setRedis(`${RedisKeys.JOINED_CONTEST_DETAILS}${jcd.match_id}-${jcd.sport}-${jcd.user_id}-${jcd.contest_id}`, `${jcd._id}`, jcd);
                                } else {
                                    // In else case delete data from s3 and redis for this for loop completely.
                                    for(const seKey of s3keyArr) {
                                        await S3Helper.deleteDataOnS3(seKey);
                                    }
                                    for(const reKey of rediskeyArr) {
                                        redisEnt.redisObj.del(reKey);
                                    }
...
async function contestAutoCreateAferJoin(contestData, series_id, contest_id, match_id, parentContestId, match_sport, liveMatch, session, matchContest, joinedContestDetailsArr) {
    try {
