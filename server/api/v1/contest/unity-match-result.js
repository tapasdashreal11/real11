        ...
        if (_.isEqual(apiKey, config.gamezop_api_key)) {
            if (rankData && rankData.length > 0) {
                ...
                if (matchTieStatus) {
                    ...
                    let matchContest = await OtherGamesContest.findOne({ 'contest_id': ObjectId(contestId), sport: match_sport });
                                ...
                                if (playerContestData && playerContestData.length > 0) {
                                    ...
                                    for (const contestTeam of playerContestData) {
                                        ...
                                        if (oPTCuserItem && oPTCuserItem.user_id) {
                                            ...
                                            await UnityGamesPtc.updateOne({ _id: contestTeam._id }, { $set: { "price_win": pricewin_amount, "points": score, "winning_amount": win_amount, "rank": rank, "winning_amount_distributed": 1, "winning_amount_notification": 1 } });
                                        }
                                    }
                                    ...
                                    await OtherGamesContest.updateOne({ contest_id: ObjectId(contestId) }, { $set: { is_distributed: 1 } });
                                    let response = {};
                                    response.success = true;
...
async function cancelContestAtResult(zop_match_id, room_id) {
    ...
    try {
        ...
        if (matchContest && matchContest._id) {
            let otherPtc = await UnityGamesPtc.find({ contest_id: ObjectId(room_id), zop_match_id: zop_match_id, is_deleted: 0, winning_amount_distributed: 0 });
            if (otherPtc && otherPtc.length > 0) {
                ...
                if (userDataList && userDataList.length > 0) {
                    ...
                    if (userArray && userArray.length > 0 && transactionArray && transactionArray.length > 0 && ptcDataArray && ptcDataArray.length > 0) {
                        ...
                        await UnityGamesPtc.bulkWrite(ptcDataArray, { session: session });
