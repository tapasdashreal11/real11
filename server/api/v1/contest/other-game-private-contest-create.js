...
const OtherGamePTC = require('../../../models/other-games-ptc');
...
module.exports = {
    createPrivateContestOG: async (req, res) => {
        try {
            let data1 = {};
            const user_id = req.userId;
            let {contest_name, contest_size, match_id, team_id, winners_count, winning_amount, entry_fee } = req.body;
            // console.log(user_id, req.body);
            // return false;
            let params = req.body;
            let constraints = { contest_name: "required", contest_size: "required",match_id: "required",team_id: "required",winners_count: "required",winning_amount: "required",entry_fee: "required" };
           ...
           // return res.send(ApiUtility.failed('Temporary stop this activity.'));
            ...
            if (decoded) {
                ...
                if (decoded['user_id'] && decoded['contest_size'] && decoded['match_id']  && decoded['winners_count']) {
                    ...
                    if (authUser) {
                        ...
                        //             matchContest['sport'] = match_sport;
                        ...
                        //     }
                            ...
                            let response = {};
                            console.log('other gamess > private contest > errorr', errorr)
                            await session.abortTransaction();
                            session.endSession();
                    ...
                    return res.send(ApiUtility.failed("match id is empty."));
                }
            } else {
                return res.send(ApiUtility.failed("You are not authenticated user."));
            }
        } catch (error) {
            console.log("Other game private contest create >>> ",error);
