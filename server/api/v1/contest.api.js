...
async function updatePTC(pctId, bonusam) {
    try {
        if (bonusam > 0) {
            var daaa2 = await PlayerTeamContest.updateOne({ _id: ObjectId(pctId) }, { "bonus_amount": bonusam });
            var daaa3 = await PlayerTeamContest.findByIdAndUpdate(ObjectId(pctId), { "bonus_amount": bonusam }, { new: true });
        }
    } catch (error) {
...
module.exports = {
    ...
    contestDetailNew: async (req, res) => {
        try {
            ...
            let reviewMatch = await SeriesSquad.findOne({ 'match_id': match_id });
            ...
            if (!contestData) {
                ...
                let myTeamIds = [];
                let customPrice = [];
                matchInviteCode = await MatchContest.getInviteCode(parseInt(match_id), contest_id);
                ...
                let myTeams = await PlayerTeamContest.getUserTeamByMatchId(match_id, contest_id, user_id);
                // let allTeams = await PlayerTeamContest.getAllTeamsByMatchId(match_id, contest_id, user_id);
                let mergedTeam = myTeams;
                let teamCount = 0;
                ...
                let akkashTeam  =   [{
                    "user_id": "5fa3a67481bab2022396b205",
                    "team_name": "AAKASHCHOPRA",
                    "user_image": "",
                    "team_no": 1,
                    "rank": 4,
                    "previous_rank": 3,
               ...
               // let teamRankData = akkashTeam.concat(teamData);
        ...
        try {
            ...
            if (mergedTeam && mergedTeam.length == 0) {
                ...
                //if (contestDetail && reviewMatch.time >= Date.now() && contestDetail.contest_size <= 50) {
                    allTeams = await PlayerTeamContest.getAllTeamsByMatchId(match_id, contest_id, user_id);
                //}
                mergedTeam = allTeams;
            }
        ...
        try {
            ...
            if (!contestData) {
                ...
                if (redisTeams) {
                    ...
                    myTeams = await PlayerTeamContest.getUserTeamByMatchId(match_id, contest_id, user_id);
                    let allTeams = await getAllTeamsByMatchIdRedis(match_id, contest_id, user_id);
                ...
                let teamRankData = MyUser.concat(newTeamData);
                //teamRankData = akkashTeam.concat(teamRankData);
                // let teamRankData = [];
    ...
    joinContestWalletAmount: async (req, res) => {
        try {
            ...
            if (userdata) {
                ...
                let redisKeyForRentation = 'app-analysis-' + decoded['user_id'] + '-' + decoded['match_id'] + '-' + match_series_id  + '-' + match_sport;
                ...
                if (decoded['invite_code'] && decoded['user_id']) {
...

                    {
                        $lookup: {
                            ...
                            let: { localteamId: "$localteam_id", seriesId: "$series_id" },
                            pipeline: [{
                                $match: {
                                    $expr: {
                                        $and: [
                    ...
                    },
                    {
                        $lookup: {
                            ...
                            // foreignField: 'team_id',
                            let: { visitorteamId: "$visitorteam_id", seriesId: "$series_id" },
                            pipeline: [{
                                $match: {
                                    $expr: {
                                        $and: [
                                            { $eq: ["$team_id", "$$visitorteamId"] },
                                            { $eq: ["$series_id", "$$seriesId"] }
                                        ]
                                    }
                                }
                    ...
                    // //////////consolelog(contestMatch);return false;
                    if (contestMatch && contestMatch.length === 1) {
                        contestMatch = contestMatch[0]
                        let authUser = await User.findOne({ '_id': decoded['user_id'] });
                        if (authUser) {
                            if (contestMatch) {
                                let seriesSquad = contestMatch.series_squad[0];
                                let totalContest = await MatchContest.find({ 'match_id': contestMatch.match_id }).countDocuments();
                                let localTeamFlag = '';
                                let visitorTeamFlag = '';
                                if (contestMatch.local_mst_team && contestMatch.local_mst_team.flag) {
                                    localTeamFlag = config.imageBaseUrl + '/team_flag/' + contestMatch.local_mst_team.flag;
                                }

                                if (contestMatch.visitor_mst_team && contestMatch.visitor_mst_team.flag) {
                                    visitorTeamFlag = config.imageBaseUrl + '/team_flag/' + contestMatch.visitor_mst_team.flag;
                                }
                                ...
                                matchData['series_name'] = contestMatch.series_name ? contestMatch.series_name : contestMatch.series.name.replace("", "Cricket ");
                                matchData['local_team_id'] = contestMatch.localteam_id;
                                matchData['local_team_name'] = contestMatch.local_mst_team.team_short_name ? contestMatch.local_mst_team.team_short_name : contestMatch.localteam;
                                matchData['local_team_flag'] = localTeamFlag;
                                matchData['visitor_team_id'] = contestMatch.visitorteam_id;
                                matchData['visitor_team_name'] = contestMatch.visitor_mst_team.team_short_name ? contestMatch.visitor_mst_team.team_short_name : contestMatch.visitorteam;;
                                ...
                                if (contestMatch.contest) {
                                    ...
                                    matchData['contest_id'] = contestMatch.contest._id;
                                    matchData['category_id'] = contestMatch.contest.category_id;
                                    matchData['entry_fee'] = contestMatch.contest.entry_fee;
                                    matchData['multiple_team'] = (contestMatch.contest.multiple_team && contestMatch.contest.multiple_team == 'yes') ? true : false;
                                    let myTeamIds = [];
                                    let teamsJoined = await PlayerTeamContest.find({
                                        'match_id': seriesSquad.match_id,
                                        'contest_id': contestMatch.contest._id,
                                        'user_id': decoded['user_id'],
                                    }, { player_team_id: 1 });
                                    ...
                                    let myTeamCount = await PlayerTeam.find({ 'match_id': contestMatch.match_id, 'series_id': contestMatch.series_id, 'user_id': decoded['user_id'] }).countDocuments();
                                    matchData['is_joined'] = teamsJoined ? true : false;
