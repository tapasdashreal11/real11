...
if ( $.ui && $.ui.fancytreedeps ) {
	$.ui.fancytreedeps.warn("Fancytree: ignored duplicate include");
	return;
}
...
function _makeVirtualFunction(methodName, tree, base, extension, extName){
	// $.ui.fancytreedeps.debug("_makeVirtualFunction", methodName, tree, base, extension, extName);
...
function _subclassObject(tree, base, extension, extName){
	// $.ui.fancytreedeps.debug("_subclassObject", tree, base, extension, extName);
...
function Fancytree(widget) {
	...
	this._id = $.ui.fancytreedeps._nextId++;
	...
	treeLoad: function(ctx, source) {
		...
		if(!source){
			...
			case "html":
				...
				source = $.ui.fancytreedeps.parseHtml($ul);
...
//          timeout: 0, // >0: Make sure we get an ajax error if server is unreachable
		...
		checkbox: false,
		clickFolderMode: 4,
		debugLevel: null, // 0..4 (null: use global setting $.ui.fancytreedeps.debugInfo)
	...
	_create: function() {
		...
		for(i=0; i<extensions.length; i++){
			...
			extension = $.ui.fancytreedeps._extensions[extName];
			if(!extension){
...
// $.ui.fancytreedeps was created by the widget factory. Create a local shortcut:
FT = $.ui.fancytreedeps;

/**
 * Static members in the `$.ui.fancytreedeps` namespace.<br>
 * <br>
 * <pre class="sh_javascript sunlight-highlight-javascript">// Access static members:
 * var node = $.ui.fancytreedeps.getNode(element);
 * alert($.ui.fancytreedeps.version);
 * </pre>
 *
 * @mixin Fancytree_Static
 */
$.extend($.ui.fancytreedeps,
	...
	debugLevel: 3,            // Set to 3 by 'grunt build'
							  // Used by $.ui.fancytreedeps.debug() and as default for tree.options.debugLevel
	...
	/** Expose class object as $.ui.fancytreedeps._FancytreeClass */
	_FancytreeClass: Fancytree,
	/** Expose class object as $.ui.fancytreedeps._FancytreeNodeClass */
	 ...
	 * @example
	 * var tree = $.ui.fancytreedeps.createTree("#tree", {
	...
	debug: function(msg){
		/*jshint expr:true */
		($.ui.fancytreedeps.debugLevel >= 4) && consoleApply("log", arguments);
	},
	/** Write error message to console if debugLevel >= 1.
	 * @param {string} msg
	 */
	error: function(msg){
		($.ui.fancytreedeps.debugLevel >= 1) && consoleApply("error", arguments);
	},
	...
	fixPositionOptions: function(opts) {
		...
		if( ! $.ui.fancytreedeps.jquerySupports.positionMyOfs ) {
	 ...
	 * @example
	 * $.ui.fancytreedeps.getTree();   // Get first Fancytree instance on page
	 * $.ui.fancytreedeps.getTree(1);  // Get second Fancytree instance on page
	 * $.ui.fancytreedeps.getTree("#tree"); // Get tree for this matching element
	 *
	 * @since 2.13
	 */
	getTree: function(el){
	 ...
	 * $.ui.fancytreedeps.evalOption("foo", node, node, tree.options);
	 * // Check for node.data.bar, tree,options.qux.bar(), and tree.options.qux.bar:
	 * $.ui.fancytreedeps.evalOption("bar", node, node.data, tree.options.qux);
	 *
	 * @since 2.22
	 */
	...
	switch( $.ui.fancytreedeps.eventToString(event) ) {
		case "-":
			tree.nodeSetExpanded(ctx, false);
			break;
		case "shift+return":
	...
	info: function(msg){
		/*jshint expr:true */
		($.ui.fancytreedeps.debugLevel >= 3) && consoleApply("info", arguments);
	},
	 ...
	 * @example
		...
		$.ui.fancytreedeps.overrideMethod(ctx.options, "createNode", function(event, data) {
	...
	parseHtml: function($ul) {
		...
		$children.each(function() {
			...
			$ul = $li.find(">ul:first");
			if( $ul.length ) {
				d.children = $.ui.fancytreedeps.parseHtml($ul);
			}else{
...
//            FT.debug("parse ", d, children);
	...
	registerExtension: function(definition){
		...
		$.ui.fancytreedeps._extensions[definition.name] = definition;
	},
	 ...
	 */
	warn: function(msg){
		($.ui.fancytreedeps.debugLevel >= 2) && consoleApply("warn", arguments);
	}
});

// Value returned by `require('jquery.fancytree')`
return $.ui.fancytreedeps;
}));  // End of closure
...
$.ui.fancytreedeps._FancytreeClass.prototype.countSelected = function(topOnly){
	var tree = this,
 ...
 */
$.ui.fancytreedeps._FancytreeNodeClass.prototype.updateCounters = function(){
	var node = this,
...
$.ui.fancytreedeps.prototype.widgetMethod1 = function(arg1){
	var tree = this.tree;
	return arg1;
};
...
$.ui.fancytreedeps.registerExtension({
 ...
 */
$.ui.fancytreedeps._FancytreeNodeClass.prototype.getCloneList = function(includeSelf){
	var key,
		tree = this.tree,
 ...
 * @requires jquery.fancytree.clones.js
 */
$.ui.fancytreedeps._FancytreeNodeClass.prototype.isClone = function(){
 ...
 * @requires jquery.fancytree.clones.js
 */
$.ui.fancytreedeps._FancytreeNodeClass.prototype.reRegister = function(key, refKey){
 ...
 * @since 2.16
 */
$.ui.fancytreedeps._FancytreeNodeClass.prototype.setRefKey = function(refKey){
 ...
 * @requires jquery.fancytree.clones.js
 */
$.ui.fancytreedeps._FancytreeClass.prototype.getNodesByRef = function(refKey, rootNode){
 ...
 * @requires jquery.fancytree.clones.js
 */
$.ui.fancytreedeps._FancytreeClass.prototype.changeRefKey = function(oldRefKey, newRefKey) {
	var i, node,
		keyMap = this.keyMap,
...
var isMac = /Mac/.test(navigator.platform),
	escapeHtml = $.ui.fancytreedeps.escapeHtml,
	unescapeHtml = $.ui.fancytreedeps.unescapeHtml;
 ...
 * @requires Fancytree
 */
$.ui.fancytreedeps._FancytreeNodeClass.prototype.editStart = function(){
	var $input,
		node = this,
		tree = this.tree,
		local = tree.ext.edit,
	...
	$.ui.fancytreedeps.assert(!local.currentNode, "recursive edit");
 ...
 */
$.ui.fancytreedeps._FancytreeNodeClass.prototype.editEnd = function(applyChanges, _event){
	var newVal,
		node = this,
		tree = this.tree,
		local = tree.ext.edit,
...
* @since 2.4
*/
$.ui.fancytreedeps._FancytreeNodeClass.prototype.editCreateNode = function(mode, init){
	var newNode,
		tree = this.tree,
		self = this;
	...
	if( init == null ) {
		...
		$.ui.fancytreedeps.assert($.isPlainObject(init));
	}
 ...
 * @requires jquery.fancytree.edit.js
 */
$.ui.fancytreedeps._FancytreeClass.prototype.isEditing = function(){
 ...
 * @requires jquery.fancytree.edit.js
 */
$.ui.fancytreedeps._FancytreeNodeClass.prototype.isEditing = function(){
...
$.ui.fancytreedeps._FancytreeClass.prototype._applyFilterImpl = function(filter, branchMode, _opts){
 ...
 */
$.ui.fancytreedeps._FancytreeClass.prototype.filterNodes = function(filter, opts) {
 ...
 * @deprecated
 */
$.ui.fancytreedeps._FancytreeClass.prototype.applyFilter = function(filter){
 ...
 * @requires jquery.fancytree.filter.js
 */
$.ui.fancytreedeps._FancytreeClass.prototype.filterBranches = function(filter, opts){
 ...
 * @requires jquery.fancytree.filter.js
 */
$.ui.fancytreedeps._FancytreeClass.prototype.clearFilter = function(){
	var $title,
		statusNode = this.getRootNode()._findDirectChild(KeyNoData),
 ...
 * @since 2.13
 */
$.ui.fancytreedeps._FancytreeClass.prototype.isFilterActive = function(){
 ...
 * @since 2.13
 */
$.ui.fancytreedeps._FancytreeNodeClass.prototype.isMatched = function(){
...
$.ui.fancytreedeps.registerExtension({
	...
	treeInit: function(ctx){
		...
		this.$container.on("focusin", function(event){
			var ctx2,
				node = $.ui.fancytreedeps.getNode(event.target);
...
var cookieStore = null,
	...
	_assert = $.ui.fancytreedeps.assert,
	ACTIVE = "active",
	EXPANDED = "expanded",
	FOCUS = "focus",
	SELECTED = "selected";
 ...
 */
$.ui.fancytreedeps._FancytreeClass.prototype.clearPersistData = function(types){
...
$.ui.fancytreedeps._FancytreeClass.prototype.clearCookies = function(types){
 ...
 * @requires jquery.fancytree.persist.js
 */
$.ui.fancytreedeps._FancytreeClass.prototype.getPersistData = function(){
	var local = this.ext.persist,
		prefix = local.cookiePrefix,
...
//		node.debug("nodeRemoveMarkup()");
	...
	nodeRender: function(ctx, force, deep, collapsed, _recursive) {
		...
		if( tree._enableUpdate === false ) {
			// $.ui.fancytreedeps.debug("*** nodeRender _enableUpdate: false");
			return;
		}
		if( !_recursive ){
			ctx.hasCollapsedParents = node.parent && !node.parent.expanded;
		}
		// $.ui.fancytreedeps.debug("*** nodeRender " + node + ", isRoot=" + isRootNode, "tr=" + node.tr, "hcp=" + ctx.hasCollapsedParents, "parent.tr=" + (node.parent && node.parent.tr));
		if( !isRootNode ){
			if( node.tr && force ) {
				this.nodeRemoveMarkup(ctx);
			}
			if( !node.tr ) {
				...
				// $.ui.fancytreedeps.debug("*** nodeRender " + node + ": prev: " + prevNode.key);
				_assert(prevNode);
...
//	        alert("treeSetFocus" + ctx.tree.$container);
		...
		$.ui.fancytreedeps.focusTree = ctx.tree;
	}*/
});
// Value returned by `require('jquery.fancytree..')`
return $.ui.fancytreedeps;
...
$.ui.fancytreedeps.registerExtension({
	...
	treeInit: function(ctx){
		...
		$el.delegate(".fancytree-node", "mouseenter mouseleave", function(event){
			var node = $.ui.fancytreedeps.getNode(event.target),
 ...
 * Private functions and variables
 */
// var _assert = $.ui.fancytreedeps.assert;
...
//  */
// $.ui.fancytreedeps._FancytreeClass.prototype.wideUpdate = function(){
